...
class BinderComponent {
  constructor(option) {
    ...
    this.numComponent = new NumericInput({
      elem: this.elem.querySelector(`.slider_show`),
    });
  ...
  halfVolume(event) {
    ...
    this.sliderComponent.setValue(
      (this.sliderElem.offsetWidth - this.thumbElem.offsetWidth) / 2,
    );
  }
}
